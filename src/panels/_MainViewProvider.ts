...
export class MainViewProvider implements WebviewViewProvider {
  ...
  public updateAppState() {
    ...
    if (this.maskConfig.pathMaskJSON === undefined) {
      ...
      if (parseResult.success) this.sendEffects();
      else {
    ...
    if (this._view) {
      if (this.maskConfig.maskJSON !== undefined) {
        ...
        this._view.webview.postMessage({
          type: "updateEffects",
          effects: this.maskConfig.maskJSON?.effects,
        });
      } else {
  ...
  public requestLogs() {
    if (this._view) {
      ...
      return new Promise((resolve) => {
        this.logEvent = this._view?.webview.onDidReceiveMessage((message) => {
          this.logEvent?.dispose();
          print("receivening logs");
          if (message.type === "returnLogs") resolve(message);
        });
      });
    } else {
      print("view is null ");
